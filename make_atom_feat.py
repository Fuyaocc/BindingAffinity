...
if __name__ == '__main__':
    ...
    files=os.listdir("../general-set_contact_graph/")
    pdb_atom_graph=set()
    for file in files:
        pdbname=file.split('_')[0]
        pdb_atom_graph.add(pdbname)
    files=os.listdir("../general-set_feat/")
    ...
    for pdbname in complexdict.keys():
        ...
        # local redisue
        with open('../general-set_contact_inside_graph/'+pdbname+'_connect.picke', 'rb') as file:
            connect = pickle.load(file)
        node_feature={}
        logging.info("generate graph :"+pdbname)
        for pdb_chains in complexdict[pdbname]:
            with open('../general-set_feat/'+pdb_chains+'.picke', 'rb') as file:
        ...
        torch.save(x.to(torch.device('cpu')),'./data/atom_graph/general-set/'+pdbname+"_x"+'.pth')
        torch.save(edge_index.to(torch.device('cpu')),'./data/atom_graph/general-set/'+pdbname+"edge_index"+'.pth')
        torch.save(edge_attr.to(torch.device('cpu')),'./data/atom_graph/general-set/'+pdbname+"_edge_attr"+'.pth')