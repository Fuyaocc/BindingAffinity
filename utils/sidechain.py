...
def  sidechain_center(dataset_path,pdbs_path,out_path):
    pdbs = []
    ...
    for pdb in pdbs:
        print(pdb)
        parser = PDBParser()
        structure = parser.get_structure(pdb, pdbs_path+pdb+'/ranked_0.pdb')  # 替换为实际的 PDB 文件路径
        ...
        for chain in model:
            ...
            if pdb+'_'+chain_name+'_sidechain_center.picke' in exist_files:continue
            sidechain=[]
...
def sidechain_angle(seq_path,out_path):
    pdbs=[]
...
if __name__ == '__main__':
    ...
    sidechain_center(dataset_path,pdbs_path,out_path)
